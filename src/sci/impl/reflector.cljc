(ns sci.impl.reflector
  "Minimal JVM reflection support for SCI.
   This is a port of parts of clojure.lang.Reflector customized for SCI's needs."
  {:no-doc true}
  #?(:clj
...
;; The following is the core custom functionality for SCI:
;; invokeMatchingMethod with type hint support
   ...
   (defn- box-arg
     ...
     (if (and (instance? IFn arg)
       ...
       (cond
         ...
         (let [n ^Number arg]
           (cond
             (= param-type Integer/TYPE) (.intValue n)
             (= param-type Float/TYPE) (.floatValue n)
             (= param-type Double/TYPE) (.doubleValue n)
             (= param-type Long/TYPE) (.longValue n)
             (= param-type Short/TYPE) (.shortValue n)
             (= param-type Byte/TYPE) (.byteValue n)
             :else (throw (IllegalArgumentException.
                           (str "Unexpected param type, expected: " param-type
                                ", given: " (.getName ^Class (.getClass arg)))))))
         ...
         (let [arg (aget args i)]
           (if (some? arg)
             (let [val-class (.getClass ^Object arg)]
               (aset widened i
                     (cond
                       (or (= val-class Integer) (= val-class Short) (= val-class Byte))
...
#?(:clj
   (defn invoke-matching-method
     ...
     ([method-name methods target args]
        ...
        (if (empty? methods)
          ...
          (let [^Method m (if (= 1 (count methods))
            ...
            (if (nil? m)
              ...
              (let [accessible-m (if (or (not (Modifier/isPublic
                                   ...
                                   ;; Use the static method from clojure.lang.Reflector directly
